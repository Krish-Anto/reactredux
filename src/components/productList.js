...
//       const response = await axios.get(`${productdata}`, {method : "GET"});
...
//   useEffect(()=> getProducts(),[]) //to call the getproducts functions one tim
    ...
    {productList.map((product,index)=>(
        <Product key ={index} product={product} id = {product.id}
         />
    ))}
  </div>
    )
